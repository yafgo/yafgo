...
func (rp *TplToyLayout) selectRepo() (repo string, err error) {
	repos := []struct {
		Name string
		Desc string
		Repo string
...
func (rp *TplToyLayout) renameModule(moduleName, dir string) (err error) {

	err = file.WalkFiles(dir, func(elem string) error {
		if strings.HasSuffix(elem, ".go") || path.Base(elem) == "go.mod" {
			// *.go, go.mod
			err := file.ReplaceString(elem, "go-toy/toy-layout", moduleName)
			return err
		}
